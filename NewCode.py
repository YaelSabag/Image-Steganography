...
def caesar_cipher(raw_text, key):
    ...
    return(cipher_text)
def decrypt(msg):
    shift = int(input(PURPLE+'Please enter its shift value: '+WHITE))
    return caesar_cipher(msg,26-shift)
...
def Decode(src,choice):
    ...
    if "$t3g0" in message:
        ...
        print(PURPLE+"Hidden message:"+WHITE, message)
    else:
...
if __name__ == "__main__":
    ...
    if userInput == '1':
        ...
        choice = input("\n{0}Please choose one option:\n1.{1} Regular security decoding.\n{0}2.{1} High security decoding.\n{1}".format(PURPLE,WHITE))
        if choice!='1' and choice!='2':
            print(RED+"ERROR: Input is wrong"+WHITE)
            exit(1)
        else:
            Decode(imageName,choice)
